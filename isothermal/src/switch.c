...
char *parameter_file;
{
  ...
  for (i = 1; i < argc; i+=d) {
    d=1;
    if (*(argv[i]) == '-') {
      if (strspn (argv[i], "-acegijkmnopqrstvxzh") != strlen (argv[i]))
	PrintUsage (argv[0]);
      if (strchr (argv[i], 'o')) {
	RedefineOptions = YES;
