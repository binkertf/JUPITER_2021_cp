     ...
     real point[3];
{
  ...
  return IC_Density(point)*r*IC_Vazimuth(point);
}
...
real MomentumFluxRotating (point)
     real point[3];
{
  real radius, azimuth, colatitude;
  real z, r;
  ...
  if (__SPHERICAL)
    r = sin(colatitude) * radius;
  return IC_Density(point)*r*(r*r*IC_Vazimuth(point)*(IC_Vazimuth(point)+OMEGAFRAME) \
				   +IC_Energy(point));
}
