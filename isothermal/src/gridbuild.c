...
//printf ("i=%ld grid->ncorner_min[j]=%ld\n",i,grid->ncorner_min[j]);
...
/*if (j==2){printf ("i=%ld k=%ld frac=%lg grid->gncorner_min[j]=%ld grid->Edges[j][k]=%lg \n",i,k,frac,grid->gncorner_min[j],grid->Edges[j][k]);}*/
       }
    }
    grid->minCPU = 0;
