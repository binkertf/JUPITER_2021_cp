  ...
  for (i = 0; i < 3; i++) {
    ...
    grids[0].nc_max[i] = (long)round((real)(grids[gtr].size[i])*\
			      ...
			      (grids[gtr].xmax[i]-grids[gtr].xmin[i])+0.01)*\
                              resol[i]+grids[gtr].nc_min[i];
//    grids[0].nc_min[i] = 2*(long)round((((real)(grids[gtr].size[i])*\
				 (grids[0].xmin[i]-grids[gtr].xmin[i])/\
				 (grids[gtr].xmax[i]-grids[gtr].xmin[i]))*\
      resol[i]+grids[gtr].nc_min[i])/2.);
//    grids[0].nc_max[i] = 2*(long)round((((real)(grids[gtr].size[i])*\
				 (grids[0].xmax[i]-grids[gtr].xmin[i])/\
				 (grids[gtr].xmax[i]-grids[gtr].xmin[i])+0.01)*\
      resol[i]+grids[gtr].nc_min[i])/2.);
//printf("i=%ld min=%lg max=%lg\n",i,((real)(grids[gtr].size[i])*\
				 (grids[0].xmin[i]-grids[gtr].xmin[i])/\
				 (grids[gtr].xmax[i]-grids[gtr].xmin[i]))*\
      resol[i]+grids[gtr].nc_min[i],((real)(grids[gtr].size[i])*\
				 (grids[0].xmax[i]-grids[gtr].xmin[i])/\
				 (grids[gtr].xmax[i]-grids[gtr].xmin[i])+0.01)*\
      resol[i]+grids[gtr].nc_min[i]); 
