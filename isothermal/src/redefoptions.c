...
void ParseRedefinedOptions (CommandLineRedefinedOptions) 
     char *CommandLineRedefinedOptions;
{
  ...
  if (length > MAXLINELENGTH-1) 
    pError ("Very long command line. Please rebuild code after increasing MAXLINELENGTH");
  if (length < 3)
    OptionError ();
