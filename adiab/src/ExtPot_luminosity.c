     ...
     real GlobalDate;
{
  ...
  real temp1=0.0,ttemp1=0.0,temp2=0.0,ttemp2=0.0,count2=0.0,count=0.0;
  real dr,daz,dco;
  FILE* sFile;
...
// This part is for monitoring the mass in the innermost cells:
 if  ( fp->desc->level < 4 && fp->desc->level > 1 ){
   for (k = Nghost[2]; k < gncell[2]-Nghost[2]; k++) {
	...
	if(dist > 0.0695214-(dr/2.0) && dist < 0.0695214+(dr/2.0) && fp->desc->level ==2) {
	    count=count+1.;	
	    temp1=temp1+temp[m];
	}
        if(dist > 0.0695214-(dr/2.0) && dist < 0.0695214+(dr/2.0) && fp->desc->level ==3 ) {
	    count2=count2+1.;	
	    temp2=temp2+temp[m];
	}
      }
