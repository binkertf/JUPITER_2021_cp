     ...
     FluidPatch *f;
{
  ...
  for (i = 0; i < n1; i++) {
	...
	if (!Isothermal) {
	  corr_energyflux1 = IC_Corr (pcmin,pcmaxr,EnergyTotFluxInertial,nsteps,pmidr);
	  corr_energyflux2 = IC_Corr (pcmin,pcmaxr,EnergyTotFluxSolidRotation,nsteps,pmidr);
	}
      } else if (NDIM == 3) {
	...
	if (!Isothermal) {
	  corr_energyflux1 = IC_2D_Mean (pcmin, pcmaxt, EnergyTotFluxInertial, nsteps, 1);
	  corr_energyflux1 /= EnergyTotFluxInertial (pmid);
	  corr_energyflux2 = IC_2D_Mean (pcmin, pcmaxt, EnergyTotFluxSolidRotation, nsteps, 1);
	  corr_energyflux2 /= EnergyTotFluxSolidRotation (pmid);
	}
      }
