...
// if (fp->desc->level == 6)  SMOOTHING=0.125*0.003379068;
 if (fp->desc->level == 6)  SMOOTHING=0.003379068;
    ...
    if (SmoothTaper == YES && fp->desc->level == LevMax){
      if(GlobalDate<0.5*3.14159+GlobalDateInit){
	// smooth-taper for half orbit of the planet
	SMOOTHING = (2.*SMOOTHORIG-limit) *
	pow(cos((GlobalDate-GlobalDateInit)/1),2)+limit; 
      }
    }
