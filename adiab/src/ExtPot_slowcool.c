...
// cutting off circumstellar disk continously:
   for (k = Nghost[2]; k < gncell[2]-Nghost[2]; k++) {
	...
	m = i*stride[0]+j*stride[1]+k*stride[2];
        ...
        if  (fp->desc->level == 0){
	    if(center[_RAD_][m] < 0.85 || center[_RAD_][m] > 1.2) {
//cut down density in the circumstellar disk:
    		  CurrentFluidPatch->Density[m] = CurrentFluidPatch->Density[m]*0.999;
	    }
        }
      }
    }
