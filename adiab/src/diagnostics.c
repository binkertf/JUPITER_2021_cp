...
/*
  ...
  Returns either the total torque, the torque from the inner or outer disk, or
  the torque density at a given colatitude, depending on the value of the Side parameter.
*/
  ...
  for (k = Nghost[2]; k < gncell[2]-Nghost[2]; k++) {
	...
	if (!(OverlapExclude  && Fluid->desc->Hidden[m])) {
	  ...
	  if ((Side==0) || ((Side==+1) && (radius>1.)) || ((Side==-1) && (radius<=1.)) || (Side==2)) {
	    ...
	    } else {    // If we are near the present colatitude, account for a fraction of the current torque.
	      // The pow(2,) come from the nesting. colatitude is cell-centered.
...
/*
  ...
  Can computes the total torque and the torque from the inner and outer disk, or the 
  torque density as a function of colatitude (depending on the value of Nsides).
*/
void TotalTorque (totaltorques, Nsides, sides, Nlimits, limits, MinCellSize)
     real *totaltorques;
     int Nsides;// if Nsides == 1, then Nlimits contains Nz, and limits contains the colatitudes
     int sides[];
     int Nlimits;
     real limits[];
     real MinCellSize;
