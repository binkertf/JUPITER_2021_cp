     ...
     real GlobalDate;
{
  ...
  real massdiv, massori, factor_taper;
  real *center[3];
  real SMOOTHORIG;
  real limit, jup_2rad;
  real  cut_dens,mass=0.0,cellvolume, tmass=0.0,cut_dens2,mass2=0.0, tmass2=0.0;
  real temp1=0.0,ttemp1=0.0,temp2=0.0,ttemp2=0.0,cumass=0.0,tcumass=0.0;
  FILE* sFile;
...
// This part is for monitoring the mass in the innermost cells:
   for (k = Nghost[2]; k < gncell[2]-Nghost[2]; k++) {
	...
	m = i*stride[0]+j*stride[1]+k*stride[2];
     ...
     massdiv=1.0;
//      massori=0.0;
      factor_taper=0.1;  // what fraction of half-orbit
      if(GlobalDate<factor_taper*3.14159+GlobalDateInit && GlobalDateInit > 0.0){
	// smooth-taper for half orbit of the planet
//	massdiv = (2.*massdiv-massori) * pow(cos((GlobalDate-GlobalDateInit)/2.),2)+massori; 
	massdiv =  1.0-(pow(cos((GlobalDate-GlobalDateInit)/2*factor_taper),2)); 
      } 
      
      if(GlobalDate>=factor_taper*3.14159+GlobalDateInit ){
           massdiv=0.0;
      }
...
/*     		  if  (CurrentFluidPatch->Density[m]> 4.e+01) {
	    ...
	    cumass=cumass+CurrentFluidPatch->Density[m]/InvVolume[m];
//printf("%g\n",massdiv);
//printf("%g %g\n",massdiv,CurrentFluidPatch->Density[m]);
	    mass2=mass2+cut_dens2/InvVolume[m];
	    CurrentFluidPatch->Density[m]=CurrentFluidPatch->Density[m]*(massdiv);
	    energ[m]=energ[m]*(massdiv);
//printf("%g  %g  %g  %g  %g  %g\n",mass2,cumass,massdiv,InvVolume[m],GlobalDate,GlobalDateInit);
	    temp2=temp2+temp[m];
	    }
        }
      }
  ...
  MPI_Allreduce (&cumass, &tcumass, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  if  (CPU_Rank == 0 && fp->desc->level == LevMax ) {
  pInfo ("Planet mass %g, mass with 2 cell radius: %g at date %.15g\n", 2.0*tcumass,2.0*tmass2,GlobalDate);
