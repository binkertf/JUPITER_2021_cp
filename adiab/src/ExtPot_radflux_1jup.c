     ...
     real GlobalDate;
{
  ...
  diffcoeff = CurrentFluidPatch->Diffcoeff;
...
// Monitor the radiative flux (~= luminosity) fluctuations on a surface during the kappa mechanism:
			...
			dr=center[_RAD_][stride[1]]-center[_RAD_][0];
	    		daz=center[_AZIM_][1]-center[_AZIM_][0];
	    		dco=center[_COLAT_][stride[2]]-center[_COLAT_][0];
	if(dist > 0.0695214-(dr/2.0) && dist < 0.0695214+(dr/2.0)) && fp->desc->level < 4 && fp->desc->level > 1 ) {
	    erad_hill=erad_hill+diffcoeff[m];
	}
	if(fp->desc->level == 3 && k == Nghost[2]&& k == gncell[2]-Nghost[2]-1 && j == Nghost[1]&& j == gncell[1]-Nghost[1]-1 && i == Nghost[0] && i == gncell[0]-Nghost[0]-1 ) {
