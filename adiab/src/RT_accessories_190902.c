...
void ComputeOpacity() {
  ...
  constDTG = YES;
  if (!constDTG){
    opacity =  (dustdens[l]/gasdens[l])*100.*kappa(temper[l],dens[l]); 
    opas[l] = (dustdens[l]/gasdens[l])*100.*3.5*RHO0*R0;  //0.1*opacity; this is the stellar opacity	
  }else{
  ...
  for (h = 0; h < gncell[2]; h++) {
    for (i = 0; i < gncell[1]; i++) {
      for (j = 0; j < gncell[0]; j++) {
          ...
          dusttemper[l] = gastemper[l]; //we set the dusttemperature equal to the gastemperature
