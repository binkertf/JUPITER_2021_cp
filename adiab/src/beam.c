     ...
     Beam *beam;
{
  ...
  for (k = 0; k < len; k++) {
    if (Isothermal || (strncasecmp(fw->Fluid->Name, "dust", 4) == 0))
     ...
     Beam *beam;
{
  ...
  for (k = 1; k < gncell[dim]; k++) {
    ...
    if (__SPHERICAL) { // here we apply the geometric correction terms from the spatial derivative to the diffusion flux
      if (dim == _COLAT_){
        radius = beam->radius;
        colatitude = fw->desc->Edges[_COLAT_][k];
        //fw->Flux_diff[dim][m] *= 1.0/radius;
      }
      if (dim == _AZIM_){
        radius =  beam->radius; //here the factor sin(theta) is included in 'radius'
        //fw->Flux_diff[dim][m] *= 1.0/radius;
      }
    }
