     ...
     real sigma0, a, h, f; 	// sigma0, sigmaslope, aspect ratio and flaring index
{
  ...
  case _energy_:
    ...
    init = h*h*pow(radius * sin(colatitude), -2.*b);
    if (!Isothermal) {
      if (NDIM == 3) {
     ...
     real sigma0, a, h, f; 	// sigma0, sigmaslope, aspect ratio and flaring index
{
  ...
  alpha = VISCOSITY/SCHMIDTNUMBER/(omegakep*hg*hg);//*(pow(radius,0.2))*1.8;
  ...
  case _vazimuth_:		// Same in 2 and 3D
    ...
    if (diffmode == 1){
      init = pow(radius, -1.5) * sqrt(1.0-3.0/2.0*VISCOSITY/SCHMIDTNUMBER/St_mid*pow(radius, -0.25));
    }else{
      init = pow(radius, -1.5);
    }
    break;
  case _energy_:
    ...
    break;
  case _vrad_:			// Viscous drift
    init = 3.0*VISCOSITY/SCHMIDTNUMBER/radius*(xi-1.5);
    if (NDIM == 2)
      init = 3.0*VISCOSITY/SCHMIDTNUMBER/radius*(a-.5);
    break;
  case _vcolatitude_:
  init = 0.0;
    break;
     ...
     real sigma0, a, h, f; 	// sigma0, sigmaslope, aspect ratio and flaring index
{
  ...
  case _vazimuth_:		// Same in 2 and 3D
    init = pow(radius, -1.5) * sqrt(1.0-3.0/2.0*VISCOSITY/SCHMIDTNUMBER/STOKESNUMBER*pow(radius, -0.25));

    break;
  case _energy_:
      init =  VISCOSITY/SCHMIDTNUMBER / STOKESNUMBER * omegakep;
    break;
  case _vrad_:			// Viscous drift
      init = 0.0; 
    break;
