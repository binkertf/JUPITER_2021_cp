#include "jupiter.h"
//  opacity routine: Judit's opacity function, modified Bell & Lin.
//  For the dust opacities, 40% silicate, 40% water ice and 20% carbon was used.
...
inline real opjudit(real temp,real rho) {
  ...
  if( temp < t234*pow(rho,power1)){
    ...
    o1=ak1*t2*temp+bk1*t2;
    o2=ak2*t2+temp*bk2;
    o3=ak3*t2*temp+bk3*t2+ck3*temp;
  ...
  if (temp > t456*pow(rho,power2)){
    if (temp < t678*pow(rho,power3) || rho <= 1.0e-10){
      ...
      //   disjoint opacity laws for 5, 6, and 7. gas opacity
      o5=bk5*rho23*ts42*ts4;
...
real kappa(real temp,real rho) {
  ...
  if(OPACITYLAW == 0){
    kappa = opjudit(temp*TEMP0,rho*RHO0);
   }
  ...
  // Opacity limiter
    if(kappa <= 0.01)
     kappa = 0.01; // This was Bertram Bitsch "fix" to avoid too low opacities for the upper layer of the disk, when there is stellar irradtion. However, it just cause harm, if one does not have stellar irradiation, because you limit the minimal opacity! So use it only on your own risk.
...
void ComputeOpacity() {
  ...
  getgridsize (fw->desc, gncell, stride);
  for (i = 0; i < 3; i++)
    size[i] = dustfluid->desc->gncell[i];
